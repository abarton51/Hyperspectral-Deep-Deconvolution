...
class Evaluator:
    ...
    def plotter(self, mono, pred, gt, blur, filename):
        for i in range(self.samples):
            ...
            axs[2, 0].imshow(blurmono, cmap='grey')
            axs[2, 0].axis("off")
            axs[1, 0].imshow(blurmono, cmap='grey')
            axs[1, 0].axis("off")
            axs[0, 0].imshow(grey, cmap='grey')
            axs[0, 0].axis("off")
            ...
            plt.savefig(self.saveDirectory + '/' + filename + str(i) + '.png', bbox_inches='tight')
    ...
    def test_model(self, filename=None):
        ...
        with torch.no_grad():
            for i, (inputs, gt, blur) in enumerate(self.testLoader):
